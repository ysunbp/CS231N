...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    score = prev_h.dot(Wh) + x.dot(Wx) + b
...
def rnn_step_backward(dnext_h, cache):
    ...
    #print(np.shape(dtanh)) N*H
    #print(np.shape(db)) H*1
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dx_i, dprev_h_i, dWx_i, dWh_i, db_i = rnn_step_backward(dprev_h_i + dh[:,i,:], cache[i])
        dx[:,i,:] = dx_i
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dscore = np.concatenate((dii, dff, doo, dgg), axis = 1)
...
def lstm_backward(dh, cache):
    ...
    dprev_c_i = 0
    for i in range(T-1, -1, -1):
        dx_i, dprev_h_i, dprev_c_i, dWx_i, dWh_i, db_i = lstm_step_backward(dprev_h_i + dh[:,i,:], dprev_c_i, cache[i])
        dx[:,i,:] = dx_i
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
