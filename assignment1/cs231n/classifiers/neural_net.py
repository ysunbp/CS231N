...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dW2 = np.dot(np.transpose(Relu1), gradi)
    db2 = np.sum(gradi, axis = 0)
    #print(W2)
    #print(dW2)
    dW2 += 2*reg*W2
    
    dRelu = np.dot(gradi, np.transpose(W2))
    dRelu[Relu1 <= 0] = 0
    
    dW1 = np.dot(np.transpose(X), dRelu)
  ...
  def predict(self, X):
    ...
    W1, b1 = self.params['W1'], self.params['b1']
    W2, b2 = self.params['W2'], self.params['b2']
    pred_X1 = np.dot(X, W1)+b1
    pred_Relu1 = np.maximum(0, pred_X1)
    pred_matrix = np.dot(pred_Relu1, W2)+b2
